...
**300 张获奖论文图形参考 + 2 张经典复刻 · 237 篇获奖论文 · 安全 / 软件工程 / AI · PPTX + SVG**

从 2024–2026 年已公布奖项的论文中，选择值得学习和复用的科研图，复刻为可编辑的 PowerPoint 对象，并配套 SVG、原图参考、预览、出处和论文清单。重点积累复杂架构、方法流程、多层数据表达及清晰的视觉组织方式。安全合集另收录 CCS 2015 Figure 1 和 NDSS 2025 SafeSplit Figure 3 两张经典复刻，作为合集附录页。
...
| 方向 | 主 PPT | 素材数 | 论文数 |
| --- | --- | ---: | ---: |
| 安全四大 | [security.pptx](decks/security.pptx) | 102 | 78 |
| 软件工程 | [software-engineering.pptx](decks/software-engineering.pptx) | 100 | 76 |
| AI | [ai.pptx](decks/ai.pptx) | 100 | 83 |
| **合计** | **安全 103 页，软工与 AI 各 101 页，均含 1 页目录** | **302** | **237** |

主 PPT 按类型分节，适合快速浏览。安全主库包含额外的“经典复刻”分节，共 2 页。其余 300 张获奖论文图形均可在[素材索引](catalog/figures.md)下载独立 PPTX 或 SVG；独立文件保留原图画幅，方便复制到自己的幻灯片。两张经典复刻只收录于安全合集，不计入 300 张独立素材和论文统计。
...
截至 **2026-09-22**，300 张单图和三个主库的 305 页已完成渲染对照；出处、图号、文件引用、独立 SVG 和数据清单均已检查。安全主库已在 PowerPoint 原生程序中打开，确认 103 页、8 个分节，其中“经典复刻”分节含 2 页。详细状态见[验证说明](docs/VALIDATION.md)与[机器记录](validation/status.json)。

软工和 AI 主库在 PowerPoint 原生程序中的整本实机抽查仍待完成。当前快照保留这一状态，渲染、工具导入和文件选择器预览均不计作原生抽查通过。部分密集图在整页视图中字号较小，请打开独立 PPTX/SVG 放大编辑。
